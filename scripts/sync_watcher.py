    ...
    def on_modified(self, event):
        ...
        if filename == "conversations.json":
            ...
            if now - self.last_run_ulisse > self.debounce:
                ...
                # Check if we should run synthetic memory update (every 20 sessions)
                try:
                    state_path = corpus_dir / "memory_state.json"
                    processed_count = 0
                    if state_path.exists():
                        with open(state_path, "r") as f:
                            state = json.load(f)
                            processed_count = len(state.get("processed_sessions", []))
                    
                    session_files = list(Path(corpus_dir / "sessions").glob("*.json"))
                    new_count = len(session_files) - processed_count
                    
                    if new_count >= 20:
                        logging.info(f"Found {new_count} new sessions. Triggering synthetic memory update.")
                        self.run_command(["scripts/synthetic_memory.py"], "Update Synthetic Memory")
                except Exception as e:
                    logging.error(f"Error checking for synthetic memory trigger: {e}")
